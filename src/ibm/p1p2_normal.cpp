...
// - with 0, 1, 2 maladapted immigrant
// - with 0, 1, 2 maladapted local
// - with 0, 1, 2 adapted local
// - with 0, 1, 2 adapted immigrant
size_t Npatches[2][3][3][3][3];

// next to counts store lists with ids
size_t patch_ids[2][3][3][3][3][Npatch];
...
void create_kid(Individual &mother, Individual &Kid)
{
    ...
    if (mother.immigrant)
    {
        ...
        expr_h = mother.z == 0 ? 0.5 * (mother.p1_imm[0] + mother.p1_imm[1]) 
                                    :
                                    0.5 * (mother.p2_imm[0] + mother.p2_imm[1]);
    }
    else
    {
